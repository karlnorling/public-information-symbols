...
Always supply a `title` (and optionally a `description`) so screen readers can announce the symbol's meaning. The SVG always gets `role="img"` and `aria-labelledby` pointing to its `<title>`, plus `aria-describedby` when a distinct description is set. For decorative symbols, bind `aria-hidden="true"` on the component.
