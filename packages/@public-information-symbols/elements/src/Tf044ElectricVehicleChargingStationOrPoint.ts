// THIS FILE IS AUTO-GENERATED. DO NOT EDIT MANUALLY.
// Run 'yarn generate' to regenerate.

import type { SvgParts } from '@public-information-symbols/core/render';
import { PISymbolBase } from './PISymbolBase';
import type { SymbolSource } from './PISymbolBase';

const _parts: SvgParts = {"attrs":"xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 52.917 52.917\"","body":"<path d=\"M120.837 104.662h52.917v52.917h-52.917z\" style=\"fill:#fff;fill-opacity:1;stroke:none;stroke-width:2.01819\" transform=\"translate(-120.837 -104.662)\"/><path d=\"M159.358 109.462a.644.644 0 0 0-.646.645v3.474h-1.57v4.216l3.095 3.109V144.9c0 .829-.635 1.463-1.463 1.463a1.44 1.44 0 0 1-1.465-1.463v-15.4c0-1.72-1.457-3.398-3.646-3.648v-13.188c0-.695-.56-1.256-1.255-1.256H135.45c-.695 0-1.255.56-1.255 1.256v37.011h-4.898v3.103h29.263v-3.103h-4.897v-21.591c.78.21 1.646.741 1.646 1.553V144.9a3.48 3.48 0 0 0 3.465 3.463 3.477 3.477 0 0 0 3.463-3.463v-24l3.057-3.07v-4.217h-1.57v-3.506a.644.644 0 0 0-.646-.645.643.643 0 0 0-.645.645v3.506h-.516v-.032h-1.915v-3.474a.643.643 0 0 0-.644-.645m-22.03 3.975h13.202a.66.66 0 0 1 .662.663v8.078a.66.66 0 0 1-.662.662h-13.202a.66.66 0 0 1-.662-.662V114.1a.66.66 0 0 1 .662-.663m9.939 13.457-3.59 7.665 6.267-.29-9.354 10.448 3.59-7.666-6.266.29z\" style=\"display:inline;fill:#000;fill-opacity:1;stroke-width:1.8\" transform=\"translate(-120.837 -104.662)\"/>","width":"200.001","height":"200.001"};
const _title = "Electric vehicle charging station or point";
const _description = "Electric vehicle charging station or point";

export class Tf044ElectricVehicleChargingStationOrPoint extends PISymbolBase {
  static override readonly tagName = 'pi-tf-044-electric-vehicle-charging-station-or-point';

  protected override _source(): SymbolSource {
    return { parts: _parts, title: _title, description: _description };
  }
}
